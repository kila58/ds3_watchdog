...
bool D3D11Renderer::Initialize()
{
	...
	hr = fontfactory->CreateFontWrapper(this->device, L"Arial", &this->fontwrapper);
	if (FAILED(hr))
		return false;

	ID3D11Texture2D* buffer;
	this->swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), (LPVOID*)&buffer);
	this->device->CreateRenderTargetView(buffer, NULL, &this->renderview);
	buffer->Release();
